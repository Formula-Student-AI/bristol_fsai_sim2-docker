...
Inherited models must implement the `update` virtual method. This method is reponsible for updating the provided
...
This stores the various vehicle parameters in the `Param` struct defined in [vehicle_param.hpp](include/models/vehicle_param.hpp).
...
| ---- | ----- | ------ | ------- |
| `getWheelSpeeds` | [State](include/models/vehicle_state.hpp), [Input](include/models/vehicle_input.hpp) | [eufs_msgs/WheelSpeeds](https://gitlab.com/eufs/eufs_msgs/-/blob/ros2/msg/WheelSpeeds.msg) | Calculates the wheel speeds (rpm) based on the vehicle velocity. |
| `getSlipAngle` | [State](include/models/vehicle_state.hpp), [Input](include/models/vehicle_input.hpp), bool (is front?) | double | Gets the [slip angle](https://en.wikipedia.org/wiki/Slip_angle) (radians). |
| `validateInput` | [Input](include/models/vehicle_input.hpp) | void | Ensures that the vehicle steering angle, velocity and acceleration values are never above their maximum or below their minimum. If they are, the values are clipped. |
| `validateState` | [State](include/models/vehicle_state.hpp) | void | Ensures that the linear velocity is always greater than zero. If not, it is set to 0. |
| `update` | [State](include/models/vehicle_state.hpp), [Input](include/models/vehicle_input.hpp), double (timestep) | void | Updates then validates the current vehicle [State](include/models/vehicle_state.hpp) based on the model dynamics and the (validated) command [input](include/models/vehicle_input.hpp). |


## Noise

The [noise.hpp](include/models/noise.hpp) header file defines a `Noise` class. This class implements two main methods: `applyNoise` and
`applyNoiseToWheelSpeeds`. The first of these methods takes a [State](include/models/vehicle_state.hpp) object and applies Gaussian noise to every
[State](include/models/vehicle_state.hpp) attribute. The second takes a [eufs_msgs/WheelSpeeds](https://gitlab.com/eufs/eufs_msgs/-/blob/ros2/msg/WheelSpeeds.msg) message and applies Gaussian noise to each individual wheel

...
from map_lib.eufs_track_generator.track_generator import TrackGenerator

# Stuff for sending a track to the sim
import rclpy
from rclpy.node import Node

from eufs_msgs.srv import SetTrack
from eufs_msgs.msg import ConeWithCovariance
from eufs_msgs.msg import ConeArrayWithCovariance


class RandomTrackClient(Node):
    def __init__(self):
        super().__init__('random_track_client')
        self.cli = self.create_client(SetTrack, '/eufs_sim2/msg')

        i = 10
        while not self.cli.wait_for_service(timeout_sec=1.0) and i > 0:
            i -= 1
        message = ("Service not available.. trying " + str(i) +
                   " more time(s). Check if eufs_sim2 is running.")
        self.get_logger().info(message)

        if i == 0:
            self.get_logger().info("Service timed out - check if eufs_sim2 is running.")

        self.req = SetTrack.Request()

    def send_request(self, track):
        self.req.data = track
...
class TrackControls(QWidget):
    ...
    def __init__(self):
        ...
        send_to_sim_btn = QPushButton("Send to eufs_sim2")

        def send_to_sim():
            start_cones, right_cones, left_cones = TrackGenerator(settings)()

            track = ConeArrayWithCovariance()

            for point in start_cones:
                cone = ConeWithCovariance()
                cone.point.x = point.real
                cone.point.y = point.imag
                track.orange_cones.append(cone)

            for point in left_cones:
                cone = ConeWithCovariance()
                cone.point.x = point.real
                cone.point.y = point.imag
                track.yellow_cones.append(cone)

            for point in right_cones:
                cone = ConeWithCovariance()
                cone.point.x = point.real
                cone.point.y = point.imag
                track.blue_cones.append(cone)

            client = RandomTrackClient()
            client.send_request(track)
            client.destroy_node()
        send_to_sim_btn.clicked.connect(send_to_sim)

        layout.addWidget(generation_group)
        layout.addWidget(cone_placement_group)
        layout.addWidget(save_btn)
        layout.addWidget(send_to_sim_btn)
...
class EUFSTracksGUI(Plugin):
    def __init__(self, context):
        ...
        self.logger.info("EUFSTracksGUI launched")
